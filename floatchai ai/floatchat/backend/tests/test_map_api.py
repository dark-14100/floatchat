...
@pytest.fixture()
def map_client():
    """TestClient overriding get_readonly_db with a per-test mock session."""
...
class TestActiveFloats:
    def test_active_floats_returns_cached_payload(self, map_client, monkeypatch: pytest.MonkeyPatch):
        ...
        resp = client.get("/api/v1/map/active-floats")
...
class TestNearestFloats:
    def test_nearest_floats_rejects_invalid_latitude(self, map_client):
        client, _ = map_client

        resp = client.get("/api/v1/map/nearest-floats?lat=95&lon=72")
...
class TestRadiusQuery:
    def test_radius_query_rejects_excessive_radius(self, map_client):
        client, _ = map_client
    ...
    def test_radius_query_returns_counts_and_bbox(self, map_client, monkeypatch: pytest.MonkeyPatch):
        client, _ = map_client
...
class TestFloatDetail:
    def test_float_detail_not_found(self, map_client):
        client, db = map_client
        ...
        resp = client.get("/api/v1/map/floats/UNKNOWN123")
...
class TestBasinFloats:
    def test_basin_floats_invalid_region_returns_400(self, map_client, monkeypatch: pytest.MonkeyPatch):
        client, _ = map_client
        ...
        resp = client.get("/api/v1/map/basin-floats?basin_name=unknown")

        assert resp.status_code == 400
        assert "No matching region" in resp.json()["detail"]

    def test_basin_floats_returns_rows(self, map_client, monkeypatch: pytest.MonkeyPatch):
        client, db = map_client
        ...
        resp = client.get("/api/v1/map/basin-floats?basin_name=arabian")
...
class TestBasinPolygons:
    def test_basin_polygons_returns_feature_collection_and_caches(
        ...
        resp = client.get("/api/v1/map/basin-polygons")
